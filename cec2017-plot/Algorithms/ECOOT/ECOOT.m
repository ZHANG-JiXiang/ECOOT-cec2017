...
function [Convergence_curve,gBest,gBestScore]=ECOOT2(N,Max_iter,lb,ub,dim,fhd,varargin)
tic
if size(ub,1)==1
    ub=ones(1,dim).*ub;
    lb=ones(1,dim).*lb;
...
for i=1:size(CootPos,1)
    CootFitness(1,i)=feval(fhd,CootPos(i,:)',varargin{:});
...
for i=1:size(LeaderPos,1)
    LeaderFit(1,i)=feval(fhd,LeaderPos(i,:)',varargin{:});
...
while l<Max_iter+1
    ...
    for i=1:size(LeaderPos,1)
        ...
        TempFit=feval(fhd,Temp',varargin{:});
    ...
    for i=1:N
        PopFit(i) =feval(fhd,PopPos(i,:)',varargin{:});
    end
    for i=1:N
    ...
    for i=1:size(CootPos,1)
        CootFitness(1,i)=feval(fhd,newPopPos(i,:)',varargin{:});
        if(gBestScore>CootFitness(1,i))
            gBestScore=CootFitness(1,i);
            gBest=newPopPos(i,:);
        end
    ...
    for i=Ncoot:N
        TempFit=feval(fhd,newPopPos(i,:)',varargin{:});
        if(gBestScore>TempFit)
            LeaderFit(1,i)=gBestScore;
            LeaderPos(i,:)=gBest;
            gBestScore=TempFit;
